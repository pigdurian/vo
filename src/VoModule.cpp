...
bool VoEngine::init(const std::string& base, const Mat& K, const std::string& poses_root)
{
    ...
    trajCanvas_.setTo(Scalar::all(0));
    map_points_.clear();
    last_inliers_.clear();
    ...
    if (img1c.empty() || img2c.empty()) return false;
    ...
    p1.reserve(status.size()); p2.reserve(status.size());
    for (size_t i = 0; i < status.size(); ++i)
        if (status[i]) { p1.push_back(pts1[i]); p2.push_back(pts2[i]); }
    if (p1.size() < 8) return false;

    // Essential + recoverPose（带 K_）
    Mat maskE;
    Mat E = findEssentialMat(p1, p2, K_, RANSAC, 0.999, 1.0, maskE);
    if (E.empty()) return false;

    // 用 RANSAC inlier 再细化一次
    vector<Point2f> p1_in, p2_in;
    p1_in.reserve(maskE.total()); p2_in.reserve(maskE.total());
    for (int i = 0; i < (int)maskE.total(); ++i)
        if (maskE.at<uchar>(i)) { p1_in.push_back(p1[i]); p2_in.push_back(p2[i]); }
    if (p1_in.size() < 8) return false;
    ...
    last_inliers_ = p2_in;   // 便于一开始也能导出一次 2D
...
{
    if (poses_root_.empty()) return 1.0;
...
VoResult VoEngine::step(int numFrame)
{
    ...
    prevGood.reserve(status.size()); currGood.reserve(status.size());
    for (size_t i = 0; i < status.size(); ++i)
        if (status[i]) { prevGood.push_back(prevFeatures_[i]); currGood.push_back(currFeatures[i]); }
    ...
    if (prevGood.size() < 8) return refreshAndReturn(true);

    // Essential + RANSAC
    Mat maskE;
    Mat E = findEssentialMat(prevGood, currGood, K_, RANSAC, 0.999, 1.0, maskE);
    if (E.empty()) return refreshAndReturn(true);

    vector<Point2f> prevIn, currIn;
    prevIn.reserve(maskE.total()); currIn.reserve(maskE.total());
    for (int i = 0; i < (int)maskE.total(); ++i)
        if (maskE.at<uchar>(i)) { prevIn.push_back(prevGood[i]); currIn.push_back(currGood[i]); }
    if (prevIn.size() < 8) return refreshAndReturn(true);
