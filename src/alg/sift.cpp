...
// 自定义 keypoint 转换为 OpenCV 自带的 cv::KeyPoint
void keypoint::convertToKeyPoint(std::vector<keypoint> &kpts, std::vector<cv::KeyPoint> &KPts)
{
...
void pyramid::appendTo(int oct, cv::Mat &img)
{
    if (oct >= pyr.size())
        pyr.resize(oct + 1);
    pyr[oct].push_back(img.clone());
}

// 重载 [] 运算符，返回某个 octave 的所有层
...
bool sift::detect(const cv::Mat &image, std::vector<keypoint> &kpts, cv::Mat &fvec)
{
  image.copyTo(img_org);
	...
	Octaves = round(log( float(std::min(img_org.cols, img_org.rows)) ) / log(2.f) - 2.f);
	...
	// INTER_LINEAR 双线性差值
	resize(img_gray_f, img, cv::Size(img_gray_f.cols * 2, img_gray_f.rows * 2), 0, 0, cv::INTER_LINEAR);
	// 插值后滤波
	double sigma_init = sqrt(std::max(Sigma * Sigma - 0.5 * 0.5 * 4, 0.01));
	GaussianBlur(img, img, cv::Size(2 * cvCeil(2 * sigma_init) + 1, 2 * cvCeil(2 * sigma_init) + 1), sigma_init, sigma_init);
...
{
   // 先计算好sigma
	std::vector<double> sigma_i(Layers + 1);
	sigma_i[0] = Sigma;
	...
	for (int oct_i = 0; oct_i < Octaves; oct_i++)
	{
		...
		for (int lyr_i = 1; lyr_i < Layers + 1; lyr_i++) // 0    1 2 3 4 5
		{
			GaussianBlur(img_i, img_i, cv::Size(2 * cvCeil(2 * sigma_i[lyr_i]) + 1, 2 * cvCeil(2 * sigma_i[lyr_i]) + 1), sigma_i[lyr_i], sigma_i[lyr_i]);
			pyr_G.appendTo(oct_i, img_i);
			subtract(img_i, pyr_G[oct_i][lyr_i - 1], img_DoG, cv::noArray(), CV_32FC1);
			pyr_DoG.appendTo(oct_i, img_DoG);
		}

		// 降采样生成下一个octave的第一张图像
		resize(pyr_G[oct_i][Layers - 2], img_i, cv::Size(img_i.cols / 2, img_i.rows / 2), 0, 0, cv::INTER_NEAREST);
	}
}
...
void sift::findFeaturePoints(std::vector<keypoint> &kpts)
{
    kpts.clear();

    // TODO: 1. 在 DoG 金字塔中寻找局部极值点
    // 迭代变量
    ...
    // TODO: 2. 对极值点进行滤波（调用 filterExtrema）

    // TODO: 3. 计算主方向（调用 calcMainOrientation）
...
// 计算特征点主方向
void sift::calcMainOrientation(keypoint &kpt, std::vector<float> &angs)
{
    // 取出特征点信息
...
void sift::calcFeatureVector(std::vector<keypoint> &kpts, cv::Mat &fvec)
{
   // fvec是特征向量组成的矩阵 每一行是一个特征点的128维向量 先申请内存空间
	...
	for (int kpt_i = 0; kpt_i < kpts.size(); kpt_i++)
	{
		...
		radius = std::min(radius, (int)sqrt(pyr_G_i.rows * pyr_G_i.rows + pyr_G_i.cols * pyr_G_i.cols));
